...
if [[ $(uname -or) != 1.*Msys ]]; then
    command make "$@" 2> >(sed -r -e "$warning" -e "$error" -e "$note" -e "$undefined_reference_to" -e "$make" -e "$compiler_recipe") \
                       > >(sed -r -e "$warning" -e "$error" -e "$note" -e "$undefined_reference_to" -e "$make" -e "$compiler_recipe")
else
